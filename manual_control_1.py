...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            spawn_point = carla.Transform(carla.Location(x=100,y=10))
            self.player = self.world.try_spawn_actor(blueprint, spawn_point)
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        if self.recording:
            ...
            # # img=cv2.imread('out/%08d'% image.frame)
            # # print(img.shape)
            # image = np.array(image.frame)

            # cv2.imshow('', image.astype(np.uint8))
            # cv2.waitKey(1)
            # # image.save_to_disk('_out/buffer' % image.frame)
...
def main():
    ...
    argparser.add_argument(
        ...
        default='512x512',
        help='window resolution (default: 512x512)')
    argparser.add_argument(
        '--filter',
        metavar='PATTERN',
        default='vehicle.*',
